...
export default function StockDetailPage({ id }: { id: string }) {
  const [article, setArticle] = useState<StockArticle | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const data = getArticle<StockArticle>(`stock_${id}`);
    setArticle(data);
    setLoading(false);
  }, [id]);
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center">
        <div className="text-center p-12 bg-white dark:bg-zinc-900 rounded-2xl border border-brand-border dark:border-white/10 max-w-md">
          ...
          <Link href="/stocks" className="inline-flex items-center gap-2 bg-brand-blue text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-600 transition-colors">
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <Link href="/stocks" className="inline-flex items-center gap-2 text-sm font-semibold text-brand-blue hover:text-blue-600 transition-colors mb-8">
          <ArrowLeft size={16} /> Back to Stocks
        </Link>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-brand-border dark:border-white/10 overflow-hidden">
          ...
          <div className="p-8 md:p-10">
            ...
            <div className="pt-6 border-t border-brand-border dark:border-white/10 flex items-center justify-between text-sm text-brand-gray dark:text-gray-400">
              ...
              <Link href="/stocks" className="inline-flex items-center gap-1.5 text-brand-blue hover:underline font-medium">
